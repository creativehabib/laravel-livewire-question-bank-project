@php
    ...
    // ✅ উন্নত কালার কম্বিনেশন: Slate এবং Indigo
    $primaryLinkClasses = 'nav-link flex items-center gap-3 rounded-xl px-4 py-3 text-[15px] font-medium text-slate-500 transition-all duration-200 hover:bg-slate-100 hover:text-slate-900 dark:text-slate-400 dark:hover:bg-slate-700/50 dark:hover:text-white';

    // অ্যাকটিভ লিংকের জন্য সফট ইন্ডিগো ব্যাকগ্রাউন্ড
    $activeLinkClasses = 'bg-indigo-50 text-indigo-700 font-semibold dark:bg-indigo-500/15 dark:text-indigo-300';

    $submenuPanelClasses = 'mt-1 space-y-1 pl-11 pr-4';
    $submenuLinkClasses = 'nav-link flex items-center gap-3 rounded-lg px-4 py-2.5 text-sm font-medium text-slate-500 transition-all duration-200 hover:text-slate-900 dark:text-slate-400 dark:hover:text-white';

    // সাবমেনু অ্যাকটিভ কালার
    $submenuActiveClasses = 'text-indigo-600 font-semibold dark:text-indigo-400';
@endphp
...
<aside id="sidebar"
       ...
       class="fixed inset-y-0 left-0 z-50 flex w-64 -translate-x-full transform flex-col border-r border-slate-200 bg-white transition-transform duration-300 ease-in-out print:hidden dark:border-slate-700/60 dark:bg-slate-800 md:translate-x-0">

    <div class="flex items-center gap-3 px-6 py-5">
        <button id="sidebarCollapse" class="hidden text-slate-500 hover:text-slate-900 dark:text-slate-400 dark:hover:text-white md:block focus:outline-none">
            <x-heroicon-s-bars-3 class="h-6 w-6" />
        </button>

        <span class="text-lg font-bold text-slate-900 dark:text-white sidebar-text tracking-tight">Question Banks</span>
    </div>
    ...
    <nav class="flex-1 overflow-y-auto px-4 py-4 space-y-1.5">
        ...
        @if($user->isAdmin())
            <div class="my-4 border-t border-slate-200 dark:border-slate-700/60"></div>
    ...
    <div class="px-4 py-4 border-t border-slate-200 dark:border-slate-700/60 space-y-2">
        ...
        <form method="POST" action="{{ route('logout') }}" class="block">
            @csrf
            <button data-sidebar-action type="submit" class="flex w-full items-center gap-3 rounded-xl px-4 py-3 text-[15px] font-medium text-red-500 transition-colors hover:bg-red-50 hover:text-red-600 dark:hover:bg-red-500/10 dark:hover:text-red-400">
